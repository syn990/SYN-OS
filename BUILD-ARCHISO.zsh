...
# straight into airootfs/usr — the same "prebuilt for x86_64" tradeoff
# the ISO already makes for its kernel and every pacman package in it.
# This also means neither the live ISO nor the installed target ever
# needs a compiler/base-devel as a runtime dependency, and there's no
# post-install makepkg step (and no repo) to maintain for these — if one
# of these tools breaks on a future host architecture, it's rebuilt from
# source here, not patched as an installed package.
SYN_SOFTWARE_DIR="$BASE_DIR/SYN-SOFTWARE"
if [[ -d "$SYN_SOFTWARE_DIR" ]]; then
  for ProjDir in "$SYN_SOFTWARE_DIR"/*-src(N/); do
    ToolName="${ProjDir:t:r:s/-src/}"
    print -P "${BLUE}Building ${ToolName} for the live environment...${NC}"
    ToolBuild="$WORKDIR/${ToolName}-build"
    if cmake -B "$ToolBuild" -S "$ProjDir" -DCMAKE_BUILD_TYPE=Release \
        && cmake --build "$ToolBuild" \
        && DESTDIR="$PROFILE/airootfs" cmake --install "$ToolBuild" --prefix /usr; then
