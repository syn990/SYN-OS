...
6. Deploys `DotfileOverlay/` onto the target filesystem if present (`cp -r`), then explicitly re-applies the executable bit on `/usr/lib/syn-os`, `/usr/local/bin`, and the `labwc`/`waybar` skel config directories — a plain file copy doesn't always preserve it. See [Dotfile Overlay](./dotfile-overlay.md).
7. Copies every locally-authored native tool's already-built binary (`syn-crypter`, `syn-filemanager`, `syn-wifi`, the waybar module backends) from the live ISO's own `/usr/lib/syn-os` and `/usr/bin` onto the target, plus `syn-filemanager`'s `.desktop` file. Each is built exactly once, from source under `SYN-SOFTWARE/`, at ISO-build time — see [Building the ISO](./build/iso-builder.md) — not compiled per-install; this step is a plain `install -D` copy, same as everything else `pacstrapMain` deploys.
