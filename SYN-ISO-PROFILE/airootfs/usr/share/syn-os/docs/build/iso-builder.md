...
`BUILD-ARCHISO.zsh`, at the root of the `SYN-OS/` project directory, builds
...
![BUILD-ARCHISO.zsh: mode selection through to a finished ISO or a dead end](../diagrams/svg/iso-build-flow.svg)
...
otherwise (`sudo zsh ./BUILD-ARCHISO.zsh`, or `--list-builds` alone, which
...
`BUILD-ARCHISO.zsh` itself needs a real Arch environment — `mkarchiso`,
...
`BUILD-ARCHISO.zsh` exactly as described above, from inside the live
