...
- If `/usr/lib/syn-os/syn-filemanager-src` exists, it's copied to `${RootMountLocation}/usr/src/syn-filemanager` as source, not a prebuilt binary — Stage 1 builds and installs it natively inside the chroot via `makepkg`, once `cmake`/`qt6-base` are already pacstrap'd. See [syn-filemanager](./tools/syn-filemanager.md).
