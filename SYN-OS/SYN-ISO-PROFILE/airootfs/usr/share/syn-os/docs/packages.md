...
- **`SYN-OS/SYN-ISO-PROFILE/packages.x86_64`** — the flat, unstructured list `archiso`/`mkarchiso` bakes into the live ISO itself. This is what's available *before* anything is installed: the live environment you boot into and run the installer from.
- **`syn-packages.zsh`** (`SYN-OS/SYN-ISO-PROFILE/airootfs/usr/lib/syn-os/syn-packages.zsh`) — the categorized package arrays `pacstrapMain` installs onto the *target disk* during [Stage 0](./stage0.md#5-pacstrapmain-syn-pacstrapzsh). This is the actual installed system's package set: desktop, shell, dev tools, apps, fonts.
...
The Wayland desktop itself: compositor, launcher, panel, wallpaper, screenshot tools, Qt theming, and the packages `syn-filemanager` needs to build.
...
| `qt6-base` | Qt6 core libraries — `syn-filemanager` (file browser, Super+E) links against this. Not a prebuilt package for `syn-filemanager` itself: that's built from source by `syn-stage1.zsh` during install (see [Stage 1](./stage1.md)) |
...
Which one `pacstrapMain` actually installs is controlled by `PackageProfile` in [synos.conf](./synos-conf.md): `full` (the default) uses `SYNSTALL`, `minimal` uses `SYNMINIMAL`. Bootloader packages (`efibootmgr`+`systemd`, `grub`, or `syslinux`) aren't in either array — `syn-pacstrap.zsh` decides those separately based on the resolved `PartitionStrat`/`BootloaderStrat` (see [Storage Strategies](./storage-strategies.md)) and appends them to whichever profile array was selected, right before the single `pacstrap -K` call that installs everything at once.
