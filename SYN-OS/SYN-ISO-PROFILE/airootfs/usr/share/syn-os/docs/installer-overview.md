...
![synos-install, start to finish: boot, edit synos.conf, Stage 0's five-function pipeline, the chroot handoff, Stage 1's six steps, installed system](./diagrams/svg/installer-overview.svg)
...
Before running it, edit `/etc/syn-os/synos.conf`: see [synos.conf, Declarative Strategy Selection](./synos-conf.md). This is the installer's only input. There are no prompts for disk layout, filesystem, or encryption once `synos-install` starts.

## Stage 0: orchestrator

`syn-stage0.zsh` (`SYN-ISO-PROFILE/airootfs/usr/lib/syn-os/syn-stage0.zsh`) does the following, in order:

1. Sources `syn-config.zsh` (loads and validates `synos.conf`), `syn-packages.zsh` (package arrays), and `syn-ui.zsh` (splash/banner helpers).
2. Sources the disk-prep pipeline (`syn-disk.zsh`) and the package/deploy stage (`syn-pacstrap.zsh`).
...
There is no unattended/non-interactive installer mode, no TUI, and no partition-picker. `synos.conf` is edited by hand in a text editor before running `synos-install`. That's a deliberate design choice, see [Philosophy](./philosophy.md), not a missing feature.
