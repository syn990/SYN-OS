...
- **`custom/vpn`**: checks for a live `wg0` interface (`ip link show wg0`) as a proxy for "WireGuard VPN connected," polled every 5 seconds. Empty format string when down, so it disappears from the bar entirely rather than showing an explicit "off" state.

- **`custom/disk`**: JSON-mode, polled every 60 seconds, backed by `syn-bar-disk.zsh` — root filesystem used/total via `df -h /`, with `class` set to `warning` (≥75%) or `critical` (≥90%). Tooltip lists every real mount (tmpfs/devtmpfs/squashfs/overlay excluded). Clicking it opens `foot -e spf /` (Superfile at the root).
