...
**`bootstrap_or_relaunch_theme()`** — branches on whether `~/.config/syn-os/current-theme` already exists:
- If it exists (not first login): sources the theme via `syn_theme_load` (from `syn-theme-lib.zsh`) and relaunches only the wallpaper (`swaybg -i "$SYN_WALLPAPER"`, backgrounded) — re-running the full `syn-theme-apply` on every login would be redundant since the theme is already applied everywhere.
- If it doesn't exist (first login): runs `syn-theme-apply SYN-OS-RED` to bootstrap the default theme into every consumer's config for the first time.

After both functions run, the file starts `mako &` (backgrounded — every `notify-send` call in SYN-OS depends on mako being alive, so it has to start before anything else that might fire a toast) and finally `waybar` (foreground, the last line — this is what keeps the autostart script itself alive for the duration of the session). See [Notifications](./tools/notifications.md) for mako's own config and theming.
